functions {
  real qlearning_log(int[] ACC, real Q0, real alpha, real beta, int ntrials, int[] pair, int[] reward) {
    vector[6] Q;
    real LL;
    real logPt;
    int a;
...
model {
  ...
  for( i in 1:nsubj ){
    ACCA[i] ~ qlearning(Q0, alphaA[i], betaA[i], ntrials, pairA[i], rewardA[i]);
    ACCB[i] ~ qlearning(Q0, alphaB[i], betaB[i], ntrials, pairB[i], rewardB[i]);
  }
}
...
generated quantities {
  ...
  for (j in 1:nsubj){
    log_lik[j]<-qlearning_log(ACCA[j], Q0, alphaA[j], betaA[j], ntrials, pairA[j], rewardA[j]) + 
                qlearning_log(ACCB[j], Q0, alphaB[j], betaB[j], ntrials, pairB[j], rewardB[j]);
  }
}